...
\end{enumerate}
\end{exercice}
1. Pour $f\in E$ fixé et $h\in E$, $t\in [0,1]$, la formule de Taylor-Lagrange donne $$g(f(t)+h(t))=g(f(t))+h(t)g'(f(t))+\dfrac{h(t)^2}{2}g''(\xi_{h,t})$$ pour $\xi_{h,t}\in \mathbb R$ de sorte que $t\mapsto \dfrac{h(t)^2}{2}g''(\xi_{h,t})$ est continue sur $[0,1]$ et $$I(f+h) = I(f) + \int_0^1 h(t)g'(f(t)) dt + \int_0^1 \dfrac{h(t)^2}{2}g''(\xi_{h,t}) dt$$ 
Le candidat pour la différentielle de $I$ en $f$ est $h\mapsto \int_0^1 h(t)g'(f(t)) dt$. \newline
Il suffit de prouver que $h\mapsto \int_0^1 h(t)g'(f(t)) dt$ est continue en $0$ et $$\dfrac{\left|\int_0^1 \dfrac{h(t)^2}{2}g''(\xi_{h,t}) dt\right|}{\|h\|}\xrightarrow[||h||\to 0]{}0$$
...
Comme $\left|\dfrac{h(t)^2}{2}g''(\xi_{h,t}) \right|\leq \dfrac{M}{2} h(t)^2$, $$\dfrac{\left|\int_0^1 \dfrac{h(t)^2}{2}g''(\xi_{h,t}) dt\right|}{\|h\|}\leq \dfrac{M}{2} \dfrac{\int_0^1 h(t)^2 dt}{\|h\|}\leq \dfrac{M}{2} \dfrac{\|h\|^2}{\|h\|}= \dfrac{M}{2}\|h\|\xrightarrow[||h||\to 0]{}0$$
...
&\leq M\|f-f_0\| \|h\|
\end{aligned}$$
Donc $\dfrac{|\varphi(f)(h)-\varphi(f_0)(h)|}{\|h\|}$ est bornée par $M\|f-f_0\|$, d'où $$ \| \varphi(f) - \varphi (f_0)\|_{op} = \sup_{h} \dfrac{|\varphi(f)(h)-\varphi(f_0)(h)|}{\|h\|}\leq M\|f-f_0\| $$
Soit $\varepsilon>0$. Avec $\delta := \dfrac{\varepsilon}{M}$, $\|f-f_0\|\leq \delta \implies \| \varphi(f) - \varphi (f_0)\|_{op} \leq \varepsilon $\newline
...
\end{enumerate}
\end{exercice}
1. $\varphi$ est continue sur $\mathbb R^n$ (car linéaire à partir d'un espace de dimension finie), donc elle admet un maximum sur le compact $C$.\newline 
...
\newline 
2. Avec la formule de calcul des dérivées partielles d'une composée, $$\dfrac{\partial f}{\partial r} = \cos(\alpha)\dfrac{\partial f}{\partial x} + \sin(\alpha)\dfrac{\partial f}{\partial y}$$
et 
...
&= x + \dfrac{1}{6n^3} + o\left( \dfrac{1}{n^3}\right) 
 \end{aligned}$ \newline
Donc $f(x+\dfrac{1}{n},y) - f(x,y)\sim \dfrac{1}{6n^3}$, donc $f(x+\dfrac{1}{n},y) - f(x,y)> 0$ à partir d'un certain rang.\newline
Un calcul similaire donne $$f(x+\dfrac{1}{n},y)=x - \dfrac{1}{6n^3} + o\left( \dfrac{1}{n^3}\right) $$
Donc $f(x+\dfrac{1}{n},y) - f(x,y)< 0$ à partir d'un certain rang. \newline
...
\textopenbullet \, si $\sin(y)=-1$: dans ce cas $f(x,y)=-x$ et \newline
$\begin{aligned}[t] f(x+\dfrac{1}{n},y) &= -x -\dfrac{1}{n} + \cos\left( x+\dfrac 1n \right) \\
&= -x-\dfrac 1n + \cos(x) -\dfrac{\sin(x)}{n} + o\left(\dfrac{1}{n}\right) \\
&= -x - \dfrac{2}{n} + o\left( \dfrac{1}{n}\right) 
 \end{aligned}$ \newline
 Donc $f(x+\dfrac{1}{n},y) - f(x,y)\sim -\dfrac{2}{n}$, donc $f(x+\dfrac{1}{n},y) - f(x,y)< 0$ à partir d'un certain rang.
De même $f(x-\dfrac{1}{n},y) - f(x,y)\sim \dfrac{2}{n}$, donc $f(x-\dfrac{1}{n},y) - f(x,y)< 0$ à partir d'un certain rang. Par le même argument que précédemmment, $(x,y)$ n'est pas un extremum local. \newline
\newline 
$\bullet$ Par ailleurs, $f(n,\dfrac{\pi}2)\xrightarrow[n\to \infty]{}\infty$ et $f(-n,\dfrac{\pi}2)\xrightarrow[n\to \infty]{}-\infty$ donc $f$ n'admet pas ni maximum global ni minimum global.\newline \newline 
...
\end{exercice}
Le problème à résoudre est le suivant $$ \min_{(x,y)\in \mathbb R^2} \|(x,y)-(0,1)\| \quad \text{s.c. } y=\dfrac{x^2}{2}$$
qu'on peut réécrire, en passant au carré, $$ \min_{x\in \mathbb R} \|(x,\dfrac{x^2}{2})-(0,1)\|^2 $$
ie $$ \min_{x\in \mathbb R} x^2+(\dfrac{x^2}{2}-1)^2 $$
...
\end{exercice}
$A$ étant symétrique, elle est diagonalisable dans une base orthonormée: on dispose de $(e_1,\ldots,e_n)$ une famille orthonormée de vecteurs propres de $A$ associés respectivement aux valeurs propres $\lambda_1 \geq \ldots \geq \lambda_n$.\newline
...
\end{enumerate}
\end{exercice}
1. Sans hypothèse de concavité en $\theta$ sur $L$, tout ce qu'on peut affirmer est que si $\dfrac{\partial \log L}{\partial \theta}(\hat \theta)=0$ et $\dfrac{\partial^2 \log L}{\partial \theta^2}(\hat \theta)<0$, alors $\hat \theta$ est un maximum local de $\log L$, donc un maximum local de $L$.\newline
...
\newline
2. On note $f:(x,y)\mapsto x + 2yx + 2y -\dfrac{x^2}2 - \dfrac{y^2}{2}$ et $K=\{(x,y),\;x\geq 0,\; y\geq 0,\; x+y\leq 1\}$. $f$ est continue, $K$ est borné (inclus dans $[0,1]^2$) et fermé (intersection de fermés), donc compact. Ceci fournit l'existence du maximum cherché.\newline
...
\newline
\newline 
3. Posons $f:(x,y)\mapsto x^3+y^2$, $g_1:(x,y)\mapsto x^2+y^2-\left(\dfrac{5}{4}\right)^2$, $g_2:(x,y)\mapsto -2x-y-\dfrac 54$ et $K=\{(x,y),\; g_1(x,y)\leq 0\;, g_2(x,y)\leq 0\}$. \newline
\newline 
...
\end{figure}
\newpage 
2. \hfill \textit{Correction proposée par Théo Compérot}\newline
